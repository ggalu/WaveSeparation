...
filename = "/home/gcg/Projekte/21_WaveSeparation/2025-03-27_Waveseparation/1_plain_pulse_1bar/in.bmp"
filename = "/home/gcg/Projekte/21_WaveSeparation/2025-03-27_Waveseparation/2_blue_foam_1bar/out.bmp"
filename = "/home/gcg/Projekte/21_WaveSeparation/2025-03-27_Waveseparation/4_PC_1bar/out.bmp"
filename = "/home/gcg/Projekte/21_WaveSeparation/2025-03-27_Waveseparation/5_PC_2bar/out.bmp"
filename = "/home/gcg/Projekte/21_WaveSeparation/2025-03-27_Waveseparation/6_PC_3bar/out.bmp"
path = os.path.dirname(filename)
fac= 0.01544061302681992337
smooth_sigma = 3.0
...
params = [{'name': 'px2mm', 'type': 'float', 'value':  fac, 'step': 1.0e-3, 'siPrefix': False, 'suffix': 'mm/px'},
          {'name': 'scan rate', 'type': 'float', 'value': 200.0, 'step': 1, 'suffix': ' kHz', 'siPrefix': False},
...
def updateVelocity():
    ...
    x, y = curve_displacement.getData()
...
def makeLayout():
    ...
    layoutVR.addWidget(plotDisplacemnt)  # plot goes on right side, spanning 3 rows
...
def clickedBtnROI1(btnROI1):
    ...
    with pg.BusyCursor():
        ...
        global curve_displacement
        x =  np.arange(N) * (1.0 / p["scan rate"])
        curve_displacement.setData(x, t1.displacements * p["px2mm"])
...
def saveToFile(btn):
    x1, y1 = curve_displacement.getData() # displacement of ROI1
    
    header = "linescan frequency = %f kHz\npixel length = %f mm \ntime [msec], ROI1 displacement [mm]" % (p["scan rate"], p["px2mm"])
...
data = read_linescan_data(filename)
numFrames = len(data)
frame0 = data[0,:]
numPix = len(frame0)
...
plotDisplacemnt = pg.PlotWidget(title="displacements of ROI")
N = numFrames
x = np.arange(N) * (1.0 / p["scan rate"])
y = np.zeros(N)
curve_displacement = plotDisplacemnt.plot(x, y, pen=(0,255,0), name="displacement ROI")
curve_LinearVelocity = plotDisplacemnt.plot(x, y, pen=(255,255,255), name="velocity curve")
plotDisplacemnt.setLabel('left', "displacement", units='mm')
plotDisplacemnt.setLabel('bottom', "time", units='ms')
velRange1 = pg.LinearRegionItem([0.4,2.0])
plotDisplacemnt.addItem(velRange1)
